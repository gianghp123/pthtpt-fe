...
import { Seat } from "@/lib/typedefs/seat";
...
export default function SelectSeatPage() {
    ...
    const [customerName, setCustomerName] = useState("John Doe"); // Default for now

    const fetchSeats = async () => {
        try {
            console.log("[DEBUG] Fetching seats...");
            ...
            if (response.success && response.data) {
                console.log("[DEBUG] Data is valid, sorting...");
                // Sort seats by ID or seat_number to ensure consistent order
                const sortedSeats = response.data.sort((a: Seat, b: Seat) => a.id - b.id);
                console.log("[DEBUG] Sorted seats:", sortedSeats);
    ...
    const confirmBooking = async () => {
        ...
        try {
            const response = await SeatService.bookSeat({
                seatId: selectedSeat.seat_number,
                customerName: customerName
            });
            if (response.success) {
                await fetchSeats(); // Refresh data
                setShowModal(false);
    ...
    const releaseSeat = async (seatId: string) => {
        ...
        try {
            const response = await SeatService.releaseSeat({ seatId });
            if (response.success) {
                await fetchSeats(); // Refresh data
            } else {
            ...
            alert("An error occurred while releasing");
        }
    };

    const getSeatStyle = (seat: Seat) => {
        if (seat.status === 'BOOKED') {
            // Check if it's "my" booking? For now, we don't have user auth, so all booked seats look the same
            // unless we track "my" bookings in session/local state.
            // For simplicity, if customer_name matches, treat as "mine" (demo purpose)
            if (seat.customer_name === customerName) {
    ...
    const myBookedSeats = seats.filter(s => s.status === 'BOOKED' && s.customer_name === customerName);

    console.log("[DEBUG] Current seats state:", seats);
                ...
                <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-slate-200/50 border border-slate-100">
                    <header className="text-center mb-12">
                        ...
                        <p className="text-slate-400 text-sm mt-2">
                            [DEBUG] Seats loaded: {seats.length}
                        </p>
                    </header>
                                ...
                                <div className="space-y-3">
                                    {myBookedSeats.map((seat) => (
                                        ...
                                        >
                                            ...
                                            <button
                                                onClick={() => releaseSeat(seat.seat_number)}
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl scale-100 animate-in zoom-in-95 duration-200 text-center">
                        ...
                        <p className="text-slate-500 mb-8">
                            Are you sure you want to book seat <strong className="text-slate-900">#{selectedSeat.seat_number}</strong>?
                        </p>
